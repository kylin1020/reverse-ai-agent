...
// Note: ProtocolError is a class, we keep importing from puppeteer-core for now
import { getFormattedHeaderValue, getFormattedRequestBody, getFormattedResponseBody, getShortDescriptionForRequest, } from '../../src/formatters/networkFormatter.js';
...
describe('networkFormatter', () => {
    describe('getShortDescriptionForRequest', () => {
        ...
        it('shows correct status for request that failed', async () => {
            ...
            const result = getShortDescriptionForRequest(request, 1);
            assert.equal(result, 'reqid=1 GET http://example.com [failed - Error in Network]');
        });
        it('marks requests selected in DevTools UI', async () => {
            const request = getMockRequest();
            const result = getShortDescriptionForRequest(request, 1, true);
            assert.equal(result, 'reqid=1 GET http://example.com [pending] [selected in the DevTools Network panel]');
        });
    });
    ...
    describe('getFormattedResponseBody', () => {
        ...
        it('handles the text limit correctly', async () => {
            const response = getMockResponse();
            response.buffer = () => {
                return Promise.resolve(Buffer.from('some text that is longer than expected'));
            };
