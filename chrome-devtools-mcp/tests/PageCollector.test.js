...
describe('NetworkCollector', () => {
    ...
    it('includes redirect chain requests in the same navigation', async () => {
        const browser = getMockBrowser();
        const page = (await browser.pages())[0];
        const mainFrame = page.mainFrame();
        // Simulate a redirect: request A (302) -> request B (final)
        const redirectRequest = getMockRequest({
            navigationRequest: true,
            frame: page.mainFrame(),
        });
        const finalRequest = getMockRequest({
            navigationRequest: true,
            frame: page.mainFrame(),
            redirectChain: [redirectRequest], // Final request has redirect chain
        });
        const subsequentRequest = getMockRequest();
        const collector = new NetworkCollector(browser);
        await collector.init([page]);
        // Both requests are emitted before framenavigated
        page.emit('request', redirectRequest);
        page.emit('request', finalRequest);
        assert.equal(collector.getData(page).length, 2);
        assert.equal(collector.getData(page)[0], redirectRequest);
        assert.equal(collector.getData(page)[1], finalRequest);
        // Navigation completes
        page.emit('framenavigated', mainFrame);
        // Both redirect and final request should still be in current navigation
        assert.equal(collector.getData(page).length, 2);
        assert.equal(collector.getData(page)[0], redirectRequest);
        assert.equal(collector.getData(page)[1], finalRequest);
        // Subsequent requests should be added
        page.emit('request', subsequentRequest);
        assert.equal(collector.getData(page).length, 3);
    });
    it('handles multiple redirects in chain', async () => {
        const browser = getMockBrowser();
        const page = (await browser.pages())[0];
        const mainFrame = page.mainFrame();
        // Simulate: A (301) -> B (302) -> C (final)
        const redirectA = getMockRequest({
            navigationRequest: true,
            frame: page.mainFrame(),
        });
        const redirectB = getMockRequest({
            navigationRequest: true,
            frame: page.mainFrame(),
            redirectChain: [redirectA],
        });
        const finalRequest = getMockRequest({
            navigationRequest: true,
            frame: page.mainFrame(),
            redirectChain: [redirectA, redirectB],
        });
        const collector = new NetworkCollector(browser);
        await collector.init([page]);
        page.emit('request', redirectA);
        page.emit('request', redirectB);
        page.emit('request', finalRequest);
        assert.equal(collector.getData(page).length, 3);
        // Navigation completes
        page.emit('framenavigated', mainFrame);
        // All three requests should still be in current navigation
        assert.equal(collector.getData(page).length, 3);
        assert.equal(collector.getData(page)[0], redirectA);
        assert.equal(collector.getData(page)[1], redirectB);
        assert.equal(collector.getData(page)[2], finalRequest);
    });
});
