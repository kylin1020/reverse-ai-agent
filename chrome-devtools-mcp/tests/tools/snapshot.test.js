    ...
    describe('browser_wait_for', () => {
        it('should work', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.equal(response.responseLines[0], 'Element with text "Hello" found.');
                assert.ok(response.includeSnapshot);
            });
        });
        it('should work with element that show up later', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await page.setContent(html `<main><span>Hello</span><span> </span><div>World</div></main>`);
                await handlePromise;
                assert.equal(response.responseLines[0], 'Element with text "Hello World" found.');
                assert.ok(response.includeSnapshot);
            });
        });
        it('should work with aria elements', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.equal(response.responseLines[0], 'Element with text "Header" found.');
                assert.ok(response.includeSnapshot);
            });
        });
        it('should work with iframe content', async () => {
            ...
            <iframe srcdoc="<p>Hello iframe</p>"></iframe>`);
                ...
                assert.equal(response.responseLines[0], 'Element with text "Hello iframe" found.');
                assert.ok(response.includeSnapshot);
            });
        });
    });
